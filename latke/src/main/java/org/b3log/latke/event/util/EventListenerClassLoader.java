 ...
 */
package org.b3log.latke.event.util;
 ...
 */
public final class EventListenerClassLoader {

    /**
     * Logger.
     */
    private static final Logger LOGGER =
            Logger.getLogger(EventListenerClassLoader.class);

    /**
     * Private default constructor.
     */
    private EventListenerClassLoader() {
    }

    /**
     * Loads event listener classes(extends from {@link AbstractEventListener})
     * with the specified class directory path.
     *
     * @param classesDirPath the specified class directory path.
     * @return a list of event listener classes
     */
    public static List<Class<AbstractEventListener<?>>> loadEventListenerClasses(
            final String classesDirPath) {
        final File eventListenerClassesDir = new File(classesDirPath);

        final List<File> classFiles = getClassFiles(eventListenerClassesDir);
        final List<Class<AbstractEventListener<?>>> ret =
                loadClasses(classFiles);
...
//                }
     ...
     */
    private static List<Class<AbstractEventListener<?>>> loadClasses(
            final List<File> classFiles) {
        final List<Class<AbstractEventListener<?>>> ret =
                new ArrayList<Class<AbstractEventListener<?>>>();
        final ClassLoader classLoader =
                AbstractEventListener.class.getClassLoader();
        ...
        try {
            for (final File classFile : classFiles) {
                ...
                final Class<AbstractEventListener<?>> clazz =
                        (Class<AbstractEventListener<?>>) classLoader.loadClass(
                        className);
